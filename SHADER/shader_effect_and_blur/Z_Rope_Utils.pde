...
class Image_Manager {
  ...
  void load_image(String... img_src_path) {
    if(img_src_path.length > 0) {
      if(image_bank == null) {
        image_bank = new PImage[img_src_path.length];
      }
      for(int i = 0 ; i <image_bank.length ;i++) {
        image_bank[i] = loadImage(img_src_path[i]);
  ...
  void select_image(int which_one) {
    which_img = which_one ;
  }
  ...
  int image_bank_length() {
    return image_bank.length ;
  }


  PImage get_image() {
    if(image_bank[0] != null) {
      if(which_img < image_bank.length) return image_bank[which_img]; 
      else return image_bank[0]; 
    } else return null ;
  }

  PImage get_image(int target){
    if(target < image_bank.length) {
      /*
      PImage img = createImage(img_bank_rope[target].width,img_bank_rope[target].height, ARGB);
      img_bank_rope[target].loadPixels();
      img.pixels = img_bank_rope[target].pixels;
      img.updatePixels();
          return img;
      */
      return image_bank[target];
    } else return null;
  }
}

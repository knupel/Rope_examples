...
PGraphics pattern_marble(int w, int h, float x_period, float y_period, float turb_power, float turb_size, int turbulence, float scale) {
  float [][] mat_2D = generate_matrix_2D(w_rope_marble_matrice, h_rope_marble_matrice, 0, 1);
  return pattern_marble(mat_2D, w, h, x_period, y_period, turb_power, turb_size, turbulence, scale);
}

PGraphics pattern_marble(PImage src, int w, int h, float x_period, float y_period, float turb_power, float turb_size, int turbulence, float scale) {
  float [][] mat_2D = new float[src.width][src.height];
  for (int x = 0 ; x < src.width ; x++) {
    for (int y = 0 ; y < src.height ; y++) {
      mat_2D[x][y] = brightness(src.get(x,y)) / g.colorModeZ;
    }
  }
  return pattern_marble(mat_2D, w, h, x_period, y_period, turb_power, turb_size, turbulence, scale);
}
  ...
  if(w > 0 && h > 0) {
    ...
    for (int x = 0; x < w; x++) {
      for (int y = 0; y < h; y++) {
        ...
        for(int i = 0 ; i < 3 ; i++) {
          // if(x == 100 && y == 100) {
          //   println("turbulence xyz",i, buf_turb[i]);
          // }
